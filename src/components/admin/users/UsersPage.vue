    ...
    <main class="mt-4">
        ...
        <table class="table">
          ...
          <tbody>
            <tr
              ...
              :key="index"
              @click="cardShow=true;currentUser=user"
            >
              <!-- <td><input type="checkbox" /></td> -->
              <td>{{ user.id }}</td>
              <td>{{ user.name }}</td>
              <td>{{ user.email }}</td>
              <td>{{ user.date_of_birth.split("-").reverse().join("-") }}</td>
              <td>
                <button
                  @click.stop="
                    updateFormShow = true;
       ...
       <div class="overlay" v-if="newFormShow || updateFormShow || cardShow">
        ...
        <div class="form-popup" v-if="cardShow">
          ...
          <form class="Course-form">
            ...
            <input class="w-100 mb-3 border-0" type="text" v-model="currentUser.name" disabled/>
            <label class="w-100">Email</label>
            <input class="w-100 mb-3 border-0" type="text" v-model="currentUser.email" disabled/>
            <label class="w-100">Date of birth:</label>
            <input class="w-100 mb-3 border-0" type="text" v-model="currentUser.date_of_birth" disabled/>
            <div class="text-end">
        ...
        <div class="form-popup" v-if="newFormShow">
          ...
          <form class="Course-form" @submit.prevent="createUser(newUser)">
            ...
            <label class="w-100">Date of birth:</label>
            <input class="w-100 mb-3" type="text" v-model="newUser.date_of_birth" />
            <label class="w-100">Password:</label>
            <input class="w-100 mb-3" type="password" v-model="newUser.password" />
            <div class="text-end">
        ...
        <div class="form-popup" v-if="updateFormShow">
          ...
          <form class="Course-form" @submit.prevent="updateUser(currentUser)">
            ...
            <label class="w-100">Date of birth:</label>
            <input class="w-100 mb-3" type="text" v-model="currentUser.date_of_birth" />
            <div class="text-end">
              <button class="btn btn-primary me-2" type="submit">
                Update
              </button>
      ...
      <div class="text-center">
        ...
        <ul class="pagination">
          ...
          >
            <span class="page-link" @click="getUsers(page)">{{ page }}</span>
          </li>
        </ul>
      </nav>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getUsers(page) {
      ...
      console.dir(this.users[0].date_of_birth)
      this.meta = this.data.meta;
      this.currentPage = page
      this.usersCount = this.users.length
      this.remainsCount = this.meta.total - this.meta.per*(this.meta.pages-1)
    },
    // create user
    async createUser(data){
      ...
      this.showSuccess()
      if( this.remainsCount == 7 )
        this.getUsers(this.meta.pages+1)
      else
        this.getUsers(this.meta.pages)
    },
    // update user
